...
var PinboardFXT = {
  ...
  onpageload: function (evt) {
    ...
    if (token) {
      ...
      pinboard.postsUpdate(null, function handleResult (err, res, body) {
  ...
  readLater: function (uri, title) {
    ...
    var t;
    void (t = open('https://pinboard.in/add?later=yes&noui=yes&jump=close&url=' + encodeURIComponent(q) +
      '&title=' + encodeURIComponent(p), 'Pinboard', 'toolbar=no,width=100,height=100'));
    t.blur();
    ...
    if (prefManager.getBoolPref('extensions.pinboardff.notifications')) {
      var alertsService = Components.classes['@mozilla.org/alerts-service;1'].getService(
  ...
  bookmark: function (uri, desc, title) {
    ...
    void (open('https://pinboard.in/add?showtags=yes&url=' + encodeURIComponent(q) +
      '&description=' + encodeURIComponent(d) + '&title=' + encodeURIComponent(p),
      'Pinboard', 'toolbar=no,scrollbars=yes,width=700,height=700'));
  },
